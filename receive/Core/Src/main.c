...
typedef struct {
	uint16_t junction_temp[8];
	uint16_t average_temp[8];
} thermistor_data_t;

uint8_t read_bit(uint8_t data, int index) {
	return data & (index == 0) ? 0x01 : (index == 1) ? 0x02 : (index == 2) ? 0x04 : (index == 3) ? 0x08 : (index == 4) ? 0x10 : (index == 5) ? 0x20 : (index == 6) ? 0x40 : (index == 7) ? 0x80 : 0;
}
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  HAL_GPIO_WritePin(CTK_1_GPIO_Port, CTK_1_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(LED_1_GPIO_Port, LED_1_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pin : CTK_1_Pin */
  GPIO_InitStruct.Pin = CTK_1_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(CTK_1_GPIO_Port, &GPIO_InitStruct);
